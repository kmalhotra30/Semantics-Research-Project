...
class DatasetPreProcessor:
  ...
  def get_context_words(self, doc_id, word, word_sense, context_num):
    ...
    doc_sents = self.corpus.sents(doc_id)

    word_occurrences = []

    seen_sentence_length = 0

    doc_wsd_sent = [self.document[doc_id]['i2s'][k] for k in list(self.document[doc_id]['wsd'][word][word_sense])]

    for sent in doc_sents:
      
      if sent in doc_wsd_sent:

        word_occurrences.append(seen_sentence_length + sent.index(word))

      seen_sentence_length += len(sent)
      ...
      for doc_id in self.doc_ids[dataset_type]:
        ...
        if word in self.document[doc_id]['wsd']:
          ...
          if word_sense in self.document[doc_id]['wsd'][word]:
            ...
            masked, unmasked = self.get_context_words(doc_id, word, word_sense, context_num)
